...
use tracing::info;
use wm_common::{
  try_warn, LengthValue, RectDelta, TilingLayout, WindowRuleEvent,
  WindowState, WmEvent,
};
...
use crate::{
  commands::{
    ...
    layouts::{
      add_dwindle_window, add_grid_window, add_manual_window,
      add_master_stack_window,
    },
...
pub fn manage_window(
  ...
  // TODO - run window rules before inserting into layout
...
fn create_window(
  ...
  let (target_workspace, target_container) =
    // TODO undo clone
    if let Some(target) = target_parent.clone() {
  ...
  // Use // the original width/height of the window and optionally
  // position it in // the center of the workspace.
  // let is_same_workspace = nearest_workspace.id() ==
  // target_workspace.id(); let floating_placement = {
  ...
  // clippy::cast_possible_truncation)]   placement.clamp_size(
