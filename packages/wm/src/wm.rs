    ...
    match event {
      ...
      PlatformEvent::Window(window_event) => match window_event {
        ...
        WindowEvent::Hidden { window, .. } => {
          handle_window_hidden(&window, state, config, &config.value.general.tiling_strategy)
        }
        ...
        WindowEvent::Destroyed { window_id, .. } => {
          handle_window_destroyed(window_id, state, &config.value.general.tiling_strategy)
        }
      },
    }?;
