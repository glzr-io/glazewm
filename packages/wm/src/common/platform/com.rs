...
// Idea here is that the cloned ComIn instance lifetime is within the
// original ComIn instance lifetime
#[repr(transparent)]
...
pub unsafe trait IApplicationView: IUnknown {
  ...
  pub unsafe fn get_trust_level(&self, ptr_trust_level: LPVOID)
    -> HRESULT;
  ...
  pub unsafe fn try_invoke_back(
    &self,
    ptr_async_callback: IAsyncCallback,
  ) -> HRESULT;
  pub unsafe fn get_thumbnail_window(
    &self,
    out_hwnd: *mut HWND,
  ) -> HRESULT;
  pub unsafe fn get_monitor(
    &self,
    out_monitors: *mut *mut IImmersiveMonitor,
  ) -> HRESULT;
  ...
  pub unsafe fn get_extended_frame_position(
    &self,
    rect: *mut RECT,
  ) -> HRESULT;
  ...
  pub unsafe fn set_last_activation_timestamp(
    &self,
    timestamp: ULONGLONG,
  ) -> HRESULT;
  pub unsafe fn get_virtual_desktop_id(
    &self,
    out_desktop_guid: *mut DesktopID,
  ) -> HRESULT;
  pub unsafe fn set_virtual_desktop_id(
    &self,
    desktop_guid: *const DesktopID,
  ) -> HRESULT;
  pub unsafe fn get_show_in_switchers(
    &self,
    out_show: *mut INT,
  ) -> HRESULT;
  ...
  pub unsafe fn on_min_size_preferences_updated(
    &self,
    window: HWND,
  ) -> HRESULT;
  ...
  pub unsafe fn is_in_high_zorder_band(
    &self,
    out_is: *mut BOOL,
  ) -> HRESULT;
  pub unsafe fn is_splash_screen_presented(
    &self,
    out_is: *mut BOOL,
  ) -> HRESULT;
  ...
  pub unsafe fn enumerate_ownership_tree(
    &self,
    objects: *mut IObjectArray,
  ) -> HRESULT; // proc46
...
pub unsafe trait IApplicationViewCollection: IUnknown {
  ...
  pub unsafe fn get_views_by_zorder(
    &self,
  ...
  pub unsafe fn get_view_in_focus(
    &self,
    out_view: *mut IApplicationView,
  ) -> HRESULT;
...
impl ComInit {
  ...
  pub fn new() -> Self {
    // Only `COINIT_APARTMENTTHREADED` works correctly.
