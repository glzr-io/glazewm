...
namespace GlazeWM.Bootstrapper
{
  class Startup
  {
    ...
    private void PopulateInitialState()
    {
      ...
      Bus.Invoke(new EvaluateUserConfigCommand());
      ...
      foreach (var screen in Screen.AllScreens)
        Bus.Invoke(new AddMonitorCommand(screen));
      ...
      foreach (var windowHandle in WindowService.GetAllWindowHandles())
      {
        ...
        Bus.Invoke(new AddWindowCommand(windowHandle, targetWorkspace, false));
      }
      ...
      {
        Bus.Invoke(new SetFocusedDescendantCommand(focusedWindow));
      ...
      if (containerToFocus is Window)
        Bus.Invoke(new FocusWindowCommand(containerToFocus as Window));

      else if (containerToFocus is Workspace)
        Bus.Invoke(new FocusWorkspaceCommand((containerToFocus as Workspace).Name));
    }

    private void OnApplicationExit()
    {
      Bus.Invoke(new ShowAllWindowsCommand());
      _barService.ExitApp();
      Application.Exit();
    }
  }
